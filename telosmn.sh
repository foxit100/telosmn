...
bootstrap0=$(curl -s https://raw.githubusercontent.com/foxit100/telosmn/main/bootstrap0)
bootstrap1=$(curl -s https://raw.githubusercontent.com/foxit100/telosmn/main/bootstrap1)
bootstrap2=$(curl -s https://raw.githubusercontent.com/foxit100/telosmn/main/bootstrap2)
bootstrap3=$(curl -s https://raw.githubusercontent.com/foxit100/telosmn/main/bootstrap3)
PORT=8051
RPCPORTT=8351
...
if [ ! -f bootstrap.zip ]
then
wget $bootstrap0 && wget $bootstrap1 && wget $bootstrap2 && wget $bootstrap3 && zip -s- TELOS.zip -O /root/bootstrap.zip
rm TELOS.zip && rm TELOS.z01 && rm TELOS.z02 && rm TELOS.z03
fi
...
RPCPORT=$(($RPCPORTT+$COUNTER))
  ...
  read PRIVKEY
  echo ""
  echo -e "${GREEN}Bootstrap is unpacking...${NC}"
  fi
  CONF_DIR=/root/.transcendence_$ALIAS
  
  mkdir /root/.transcendence_$ALIAS
  7z x bootstrap.zip -o/root/.transcendence_$ALIAS >/dev/null 2>&1
  ...
  rm cron$ALIAS
  echo ""
  echo -e "${GREEN}Bootstrap has been unpacked !${NC}"
  echo ""
...
then
  echo ""
  echo -e "${RED}No more Telos masternodes can be created on this VPS !${NC}"
  echo ""
fi
...
echo -e "${GREEN}Dawid${NC} Transcendence Address for donations: ${GREEN}GSvjeBCVxcS2nyVE6KUn4uCMcY9BkT6r7j${NC}"
